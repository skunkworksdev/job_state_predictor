...
function slurm_job_submit(job_desc, part_list, submit_uid)
	...
	if submit_uid == 0 then
		if job_desc.job_id ~= nil then
			handle = io.popen("/root/submit_script.sh " .. job_desc.job_id)
			result = handle:read("*all")
			handle:close()
			_log_user_and_debug(result)
		else
                        handle = io.popen("/slurm/submit_script.sh " .. "interactive_job")
			output = handle:read("*all")
			handle:close()
			_log_user_and_debug(output)
		end
		return slurm.SUCCESS
	end
